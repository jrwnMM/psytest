...
class Statistics(LoginRequiredMixin, SuperUserCheck, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["riasec_male"] = CareerResult.objects.filter(user__profile__gender="Male").aggregate(*riasec_avg)
        context["riasec_female"] = CareerResult.objects.filter(user__profile__gender="Female").aggregate(*riasec_avg)
        ...
        context["rm_count"] = CareerResult.objects.filter(user__profile__gender="Male").count()
        context["rf_count"] = CareerResult.objects.filter(user__profile__gender="Female").count()
        ...
        context["personality_male"] = PersonalityResult.objects.filter(user__profile__gender="Male").aggregate(*personality_avg)
        context["personality_female"] = PersonalityResult.objects.filter(user__profile__gender="Female").aggregate(*personality_avg)
        ...
        context["pm_count"] = PersonalityResult.objects.filter(user__profile__gender="Male").count()
        context["pf_count"] = PersonalityResult.objects.filter(user__profile__gender="Female").count()
...
def career_male(request):
    ...
    data['avg'] = CareerResult.objects.filter(user__profile__gender="Male").aggregate(*riasec_avg)
...
def career_female(request):
    data ={}
    data['labels'] = career_labels
    data['avg'] = CareerResult.objects.filter(user__profile__gender="Female").aggregate(*riasec_avg)
    return JsonResponse(data)
...
def personality_male(request):
    ...
    data['avg'] = PersonalityResult.objects.filter(user__profile__gender="Male").aggregate(*personality_avg)
    return JsonResponse(data)
...
def personality_female(request):
    data ={}
    data['labels'] = personality_labels
    data['avg'] = PersonalityResult.objects.filter(user__profile__gender="Female").aggregate(*personality_avg)
    return JsonResponse(data)
...
def career_category(request):
    ...
    if category == '1':
        ...
        context["riasec_male"] = CareerResult.objects.filter(user__profile__gender="Male").aggregate(*riasec_avg)
        context["riasec_female"] = CareerResult.objects.filter(user__profile__gender="Female").aggregate(*riasec_avg)
        return render(request, 'statistics/partials/career_gender.html', context)
...
def personality_category(request):
    ...
    if category == '1':
        ...
        context["personality_male"] = PersonalityResult.objects.filter(user__profile__gender="Male").aggregate(*personality_avg)
        context["personality_female"] = PersonalityResult.objects.filter(user__profile__gender="Female").aggregate(*personality_avg)
        return render(request, 'statistics/partials/personality_gender.html', context)
